...
impl Decryptor {
    ...
    fn tweaked_rc4(&mut self, dec_size: usize) {
        ...
        if cur_pos <= 0x80 {
            for i in 0..0x80 {
                ...
                self.segment[i] ^= self.decrypt_key.key[pos];
            }
            self.dec_part(cur_pos + 0x80, dec_size - 0x80, 0x80);
        } else {
            let remain = PART_SIZE - cur_pos as usize % PART_SIZE;
            if dec_size > remain {
    ...
    fn dec_part(&mut self, cur_pos: u64, dec_size: usize, seg_pos: usize) {
        ...
        let rounds = cur_pos as usize % PART_SIZE + self.calc(cur_pos as usize / PART_SIZE);
        let mut i = 0;
        ...
        for j in 1..dec_size+rounds+1 {
            ...
            if j > rounds {
                self.segment[seg_pos + j - rounds - 1] ^= perm[(perm[i] as usize + perm[j % key_len] as usize) % key_len];
            }
        }
    }
}
